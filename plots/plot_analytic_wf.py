...
def eval_log_laughlin_m1(x_batch):
    """Evaluates the log of the m=1 Laughlin wave function."""
    # Convert real (x, y) to complex z
    z = x_batch[..., 0] + 1j * x_batch[..., 1]
    
    # Use vmap to apply vander across the batch dimension
    V = jax.vmap(jnp.vander)(z)
    
    # Compute log determinant stably
    sign, logdet = jnp.linalg.slogdet(V)
    
    # slogdet returns complex sign. Extract phase.
    phase = jnp.angle(sign)
    
    # Gaussian envelope: -0.5 * sum(|z_i|^2)
    envelope = -0.5 * jnp.sum(jnp.square(x_batch), axis=(-2, -1))
    
    return logdet + envelope + 1j * phase
...
def compute_overlap_squared(log_psi_nn, x_batch, N):
    ...
    # Compute overlap for Laughlin m=1
    log_psi_L = eval_log_laughlin_m1(x_batch)
    ratio_L = jnp.exp(log_psi_L - log_psi_nn)
    s2_L = jnp.square(jnp.abs(jnp.mean(ratio_L))) / jnp.mean(jnp.square(jnp.abs(ratio_L)))
    overlaps["Laughlin (m=1)"] = float(s2_L)
...
def plot_wf(system, vstate, plot_path, step=None):
    ...
    print(rf"Overlap: Laughlin (m=1) & {overlaps['Laughlin (m=1)']:.4f} \\")
    print(r"\hline")
    print(rf"\textbf{{Total Overlap}} & \textbf{{{overlaps['Total_Manifold']:.4f}}} \\")
    print(r"\hline")
    print(r"\end{tabular}")
    print(r"\caption{Energy and squared overlap of the trained NQS with the exact ground state manifold and the Laughlin state.}")
    ...
    # Density for Laughlin state
    log_psi_L_grid = eval_log_laughlin_m1(full_configs)
    prob_L = jnp.exp(2.0 * jnp.real(log_psi_L_grid))
    prob_density_L = (prob_L / jnp.max(prob_L)).reshape(100, 100)

    # 2x3 Grid to accommodate the new plots
    ...
    # 1: Laughlin Density
    cL = axes[1].contourf(X, Y, prob_density_L, levels=50, cmap="magma")
    fig.colorbar(cL, ax=axes[1])
    axes[1].set_title("Analytic Laughlin (m=1)")

    # 2-4: Analytic States A, B, C
    ...
    # Hide the 6th unused subplot to keep it clean
    axes[5].axis('off')
