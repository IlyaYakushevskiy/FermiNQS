...
As introduced in Section~\ref{sec:nn_ansatz}, the FermiSets ansatz is built from a symmetric Deep Sets embedding $\xi$ and the antisymmetric signature encoder $\eta$. Figure~\ref{fig:fermisets_architecture} shows the resulting computational graph; concretely, a forward pass proceeds as follows.

\begin{enumerate}
    \item \textbf{Input.} Particle coordinates arrive as a batch of shape $(\text{batch}, N, \texttt{dim})$; for the $\texttt{dim}=2$ systems this thesis focuses on, each particle contributes a pair $(x_i, y_i)$, packed into $z_i = x_i + iy_i$.
    \item \textbf{Signature encoder.} $\eta(R)$, the (regularised) complex Vandermonde determinant of Equation~\ref{eq:eta}, is computed directly from the raw coordinates. This is the only antisymmetric ingredient in the whole network; everything else below is built to be exchange-symmetric.
    \item \textbf{Deep Sets embedding.} In parallel, each particle's coordinates pass through the same shared encoder $\phi$ (two dense layers), are summed over particles (the permutation-symmetric pooling step), and the pooled vector is passed through a decoder $\rho$ to give $\xi(R)$, a fixed-size, permutation-invariant summary of the whole configuration, regardless of $N$.
    \item \textbf{Feature concatenation and the $\Psi$-net.} $\xi(R)$ is concatenated with $\mathrm{Re}(\eta)$ and $\mathrm{Im}(\eta)$ and fed into a single, generously wide layer (\texttt{hidden\_units} with a fixed width multiplier, rather than several narrower stacked layers). This is a deliberate design choice, not a default: the Universal Approximation Theorem (Section~\ref{sec:intro}) already guarantees that one sufficiently wide layer with a non-linearity can approximate the function we need, and empirically, adding an extra stacked layer here made training roughly $30\times$ slower per iteration for no accuracy benefit (Appendix~\ref{app:ablations}). Within this VMC setup, shallow-and-wide outperforms deep in practice, and the theorem gives us license to prefer it.
    \item \textbf{Manual antisymmetrization.} The $\Psi$-net above is evaluated \emph{twice} with shared weights: once at $+\eta$ and once at $-\eta$, giving two log-amplitudes $\Psi_0(\xi, \eta)$ and $\Psi_0(\xi, -\eta)$. These are combined explicitly as
    \begin{equation}
        \Psi(\xi,\eta) = \tfrac{1}{2}\big[\Psi_0(\xi,\eta) - \Psi_0(\xi,-\eta)\big]
    \end{equation}
    (computed in log-space via a signed log-sum-exp for numerical stability, but this equation is exactly what it computes). This is the step that actually enforces the fermionic sign flip: nothing upstream of it is antisymmetric under particle exchange on its own, only this explicit $\Psi_0(+\eta) - \Psi_0(-\eta)$ subtraction is.
    \item \textbf{Output.} The result is returned as $\log \psi$, split into a log-modulus and a phase channel, never as $\psi$ itself. NetKet's variational-state machinery expects log-amplitudes; $|\psi|$ can span many orders of magnitude across a batch, and exponentiating anywhere inside the network would overflow long before that.
\end{enumerate}
...
Here we gonna do a lot of pistures, generate wavefunctions from checkpoints! Analyse whole backlog from cluster runs. 
