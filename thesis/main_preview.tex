...
\begin{equation}
    ...
    \label{eq:fidelity}
\end{equation}
estimated by importance sampling from a Gaussian reference distribution (Section~\ref{sec:diagnostics}).
...
What training actually does is collected in Figure~\ref{fig:n3_trap}. From random initial parameters the energy falls steeply out of its initial value near $7.5\,\hbar\omega$ and settles shortly onto a plateau at $E = 6.0022 \pm 0.0026$ averaged over the last hundred iterations, and it does not leave that plateau again for the remainder of the run. Every diagnostic we have says this run is healthy. The variance of the local energy falls from $4.3$ to $0.024$, which is precisely the signature of an exact eigenstate, whose local energy is constant across configurations. The independently reseeded validation chain agrees with the training estimate inside its error bar, $6.0041 \pm 0.0028$ at iteration $350$, so the plateau is not an autocorrelation artefact of the training sampler; the acceptance rate holds at $0.87$ and $\hat R$ stays between $1.07$ and $1.10$ throughout. The trust-region rescaling of the stochastic-reconfiguration step never engages either, the bilinear form remaining in the range $8$ to $16$ against its threshold of $900$, so the optimiser is not being quietly truncated. By every criterion one would ordinarily apply to a variational run, this one has converged.

It has converged one full quantum above the ground state with value $6 = N(N+1)/2$ is the exact eigenvalue of the holomorphic Vandermonde state introduced in Section~\ref{sec:lazy_trap}, the state $\det[1, z, z^2]\,e^{-r^2/2}$ whose antisymmetric factor is the complex Vandermonde $\prod_{i<j}(z_i - z_j)$, which is to say the signature encoder $\eta$ itself. The fidelities confirm the identification without room for interpretation. Against the analytic ground state the trained wavefunction has $\mathcal{F} = 10^{-6}$, no ground-state content whatsoever, while against the holomorphic state it has $\mathcal{F} = 0.85$. 

This state has a name. It is the $\nu = 1$ Laughlin wavefunction $\Psi_{LL} = \prod_{i<j}(z_i - z_j)^q\,e^{-\sum|z_i|^2/2}$ at $q=1$, that is the filled lowest Landau level transplanted from a magnetic field into the harmonic trap. What makes it celebrated in that context is exactly what makes it a trap here: its antisymmetry is a pure product of pairwise differences, the sortable sign structure that the fixed factor $\eta$ supplies for free.
...
This gives a hint for another metric, namely the phase. TO WRITE HERE (why its important)
...
\end{equation}
The target is the exact non-interacting ground state, which for the harmonic trap is a single Slater determinant of oscillator orbitals. If the ansatz can be driven onto the ground state this way the state is representable, and that direction is a constructive proof. The converse is weaker and has to be stated carefully, because the supervised loss is itself non-convex: a stalled fit is consistent with a genuine representability limit and with a failure of the supervised optimisation alike, and no finite experiment separates the two with certainty. What the test does deliver is an upper bound on what is achievable in practice, since it removes the Monte Carlo noise and the variational energy landscape and hands the network the answer outright. We therefore read a stalled fit as evidence rather than as proof, and strengthen it by closing the remaining optimisation explanations one at a time.

For the closed-shell benchmark at $N=3$ this test comes out cleanly on the side of expressivity. The supervised loss falls, the phase term reaches $0.19$, and when we release ordinary VMC from the pretrained parameters the energy settles onto the true ground state at $E \approx 5.05$ with fidelity $\mathcal{F} \approx 0.99$ against the analytic determinant, instead of drifting back up to the trap at $E=6$. The network is therefore fully capable of representing the $N=3$ ground state; what it lacks from a real scenario is initialisation with any gradient that leads there. This is the concrete confirmation, promised in Section~\ref{sec:nn_ansatz}, that the trap at $N=3$ is a failure of trainability and not of expressivity, we thought to ourselves and moved to our other experiments on beating the trap. 

Later we found this verdict does not survive being carried to larger systems. We therefore ran the identical fit at every particle number we study. Two of them need care: $N=4$ and $N=5$ are open shells, so the ground state is degenerate and the aufbau filling picks one member of the manifold arbitrarily. We fit two distinct members at each, and at $N=4$ the genuinely distinct second choice is the $(1,1)$ orbital, since $(0,2)$ and $(2,0)$ are related by a rotation of the plane and $\eta$ is rotation covariant.

The result is in Table~\ref{tab:supervised} and Figure~\ref{fig:supervised_assay}. Alongside the two loss terms we report the squared overlap $\mathcal{F}$ of each fitted state with its target, and it is the overlap that should be read: it is an observable, and it weights configurations by $|\psi|^2$, whereas the phase term weights every configuration's angle equally and so overstates the failure wherever the state carries little probability. Read that way the division is sharp. At $N=3$ the ansatz reproduces the ground state ($\mathcal{F} = 0.91$) and at $N=4$ it still captures most of it ($0.80$), at $N=5$ only a tenth remains ($0.11$), and from $N=6$ through $N=8$ the overlap is numerically zero. The failure is not an accident of how the fits were run, and we closed each way it could have been one. It is not the optimisation budget, since quadrupling it moves $N=4$ only from $0.71$ to $0.59$, nowhere near the $0.19$ that constitutes a fit at $N=3$, and leaves $N=6$ at $0.995$. It is not decoder capacity, since doubling the hidden width at $N=6$ changes nothing. It is not the random seed: three seeds give $1.000 \pm 0.009$ at $N=6$ and $0.478 \pm 0.052$ at $N=4$, so the ceiling reproduces to about one percent. It is not the arbitrary choice of degenerate member, since both members fail at both open shells. And it is not an artefact of studying the bare network rather than the projected one, which we take up below. What survives all of this is a single ordering. The overlap tracks how many shell-$2$ orbitals the target occupies, those being the ones whose nodal structure is not holomorphic, falling from $0.91$ with none of them to $0.80$, $0.11$ and $0$ as one, two and three are filled. The spread within $N=4$ reads the same way, since shell $2$ carries $m \in \{-2,0,+2\}$ and $(1,1) \propto xy$ is pure $|m| = 2$ while $(0,2) \propto y^2$ mixes in $m = 0$, and a holomorphic $\eta$ favours maximal $|m|$.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{supervised_assay.png}
    \caption{Supervised expressivity ablation across particle number (hidden $64$ unless noted, $25{,}000$ steps). Left: phase loss against supervised step; the dotted line at $1$ marks sign structure uncorrelated with the target. Right: the converged phase loss against $N$, with open circles marking the second member of the degenerate ground-state manifold at the open shells $N=4$ and $N=5$, and the cross marking $N=6$ at hidden $128$. The degradation is monotone in $N$, the member choice shifts the value but not the trend, and width is not a lever.}
...
\begin{table}[H]
    ...
    \begin{tabular}{l l r r r}
        ...
        $N=6$, $L_z$-projected $K=6$ & all three & $1.05$ & $0.99$ & $-$ \\
        \hline
    \end{tabular}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{n3_lz_density_walk.png}
    \caption{The same construction as Figure~\ref{fig:n3_trap_walk} for the $L_z$-projected run ($K=6$, otherwise identical seed and hyperparameters). The projection empties the initial state of both candidates, and training then walks onto the true ground state, reproducing its collinearity node, rather than onto the crescent of the trap. Overlaps with the holomorphic and antiholomorphic states stay at $10^{-5}$ for the whole run.}
...
One objection has to be closed before this can be called a limitation of the method rather than of a network we never use. Every fit quoted so far is of the bare ansatz, whereas the runs of Section~\ref{sec:lz_projection} evaluate the projected function $P_K\psi$, and the two are not interchangeable: the trained $N=3$ parameters give $\mathcal{F} = 0.9988$ against the ground state when the projection is applied and $E = 7.41$ with almost no ground-state weight when it is not, so the projection of a network can be the ground state while the network itself is nowhere near it. We therefore repeated the fit on the projected ansatz. At $N=3$ it does not merely reproduce the earlier result but improves on it, reaching a phase loss of $0.07$ against $0.19$ for the bare network and an energy of $5.051 \pm 0.009$ against the exact $5$, which is what one should expect, since the sieve removes in advance the wrong-sector content the fit would otherwise have to cancel by hand. At $N=6$ the projected fit is pinned at $0.99$, indistinguishable from the bare one. The projection is therefore not the thing that fails here. It decides which sector the search occupies, it buys no nodal freedom, and nodal freedom is what is missing. The true $N=6$ ground state fills the third shell, whose orbitals $(0,2), (1,1), (2,0)$ have a nodal structure that is not holomorphic, while the entire antisymmetric content of the ansatz is carried by the fixed holomorphic factor $\eta$, whose nodes the trainable symmetric part cannot move. The network therefore cannot place the non-holomorphic nodes the ground state requires, and no amount of optimisation or width can supply what the architecture does not contain. Architectural attempts to give the ansatz that missing freedom, a pairwise-distance feature stream and a trainable equivariant deformation of the signature encoder, do not lift the ceiling either; we report them in Appendix~\ref{app:variants}, as they change none of the conclusions here.
