...
The Universal Approximation Theorem tells us that with a sufficiently large linear layer and a non-linear activation function we can approximate a function of arbitrary complexity, which is the premise of this thesis.
...
\cite{orus_practical_2014} For a system of, e.g., $N$ spins-1/2, the dimension of the Hilbert space is $2^N$, which
...
+ The Fermionic Sign Problem (we dont care)

Fermionic Sign Problem appears in VMC setups as a consequence of the Pauli principle and constitutes a big computational challenge for VMC setups.


figure targeting part of Hilbert space

 formal mathematics of the permutation symmetric network extended with the predefined antisymmetric function.

- NQS works in continuous space
Here some
\newpage

\subsection{Variational Monte Carlo}

 The base of this approach is the quantum-mechanical variational principle, one of the big advantages of which is that the energy of the system is bounded from below by the ground state energy. By optimizing our variational state with respect to parameters $\theta$ in order to decrease the energy, we hope to capture an approximation of the ground state.

 (here from Becca Sorella on VMC ) 
...
The construction above has a shortcut built into it. For any smooth symmetric function $g(\xi)$, the state

\begin{equation}
    \psi(r_1,...,r_N) = \eta(r_1,...,r_N) \cdot g(\xi(r_1,...,r_N)) \cdot e^{-\sum_i r_i^2/2}
\end{equation}

is automatically antisymmetric, since $\eta$ already flips sign under every particle exchange. The network does not have to learn anything about the sign structure to satisfy the Pauli principle -- it can simply ignore the phase of $\eta$ and output a plain symmetric prefactor. This is not a bug of a particular training run, it is a property of the ansatz itself.
