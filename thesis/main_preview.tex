...
The object \textbf{System} includes a description of the continuous Hilbert space we work with, its dimension, and the Hamiltonian (namely the potential part) for which we solve Schrödinger's equation. Concretely, this Hilbert space is NetKet's \texttt{Particle} Hilbert space over free space -- no periodic boundary conditions are implemented -- and the Hamiltonian is a fixed kinetic-energy operator plus the potential term below. It's worth noting that antisymmetrization of the state is enforced by the Ansatz (Neural Quantum State), not by the System.

Three potentials are implemented, selected via \texttt{system.potential}:
- \texttt{qho\_no\_inter}: the isotropic harmonic trap, $V(x) = \frac{1}{2}\sum_i r_i^2$ -- the main benchmark system throughout this thesis.
- \texttt{qho\_aniso}: an anisotropic trap, $V(x) = \frac{1}{2}\sum_i (x_i^2 + \omega_y^2 y_i^2)$, controlled by \texttt{system.omega\_y} -- breaks rotational symmetry deliberately, used to test whether the ansatz's behaviour is specific to the isotropic case.
- \texttt{dot\_gauss}: an interacting quantum dot, the trap plus a Gaussian repulsion $\lambda \sum_{i<j} \exp(-r_{ij}^2/2s^2)$, controlled by \texttt{system.int\_strength} ($\lambda$) and \texttt{system.int\_range} ($s$). This system has no closed-form ground state, so the reference energy \texttt{system.e\_ref} must be supplied from an exact diagonalization instead (Section~\ref{sec:diagnostics}).

Other System parameters: \texttt{N}, the number of particles; \texttt{dim}, the dimension of the continuous Hilbert space (this thesis focuses on \texttt{dim=2}); and \texttt{mass}, the particle mass (kept at 1 throughout).
...
- a Gaussian drift, $r \rightarrow r + \sigma \cdot \mathcal{N}(0,1)$, with step size \texttt{sampler.sigma};
- a particle-exchange move, swapping the full coordinates of two randomly chosen particles, proposed with probability \texttt{sampler.exchange\_prob}.
...
- \texttt{trainer.lr} is the base learning rate, decayed exponentially with rate \texttt{trainer.lr\_decay\_rate} every \texttt{trainer.lr\_decay\_steps} iterations.
- \texttt{trainer.optimizer} selects between \texttt{sgd}, \texttt{momentum} (with \texttt{trainer.momentum\_beta}), and plain \texttt{adam} (no SR, used only for the optimizer comparison in Section~\ref{sec:results}).
- \texttt{trainer.vmc\_iters} is the number of training iterations; \texttt{trainer.n\_samples} and \texttt{trainer.n\_discard\_per\_chain} set the number of Monte Carlo samples used per gradient step and the burn-in discarded from the start of each chain.
- \texttt{trainer.diag\_shift} is the diagonal shift $\lambda$ introduced in Section~\ref{sec:SR}; \texttt{trainer.pinv\_rtol}/\texttt{pinv\_atol} control the tolerance of the Cholesky-based solver used to invert the (shifted) S matrix.
- \texttt{trainer.chunk\_size} splits the batch of samples into smaller chunks for the forward/backward pass, trading speed for GPU memory -- necessary since the S matrix scales as (number of samples)$^2$ under the kernel trick (\texttt{use\_ntk=True}) used throughout.

Two safety mechanisms sit around the driver:
- a trust-region rescaling of the SR step: if the natural-gradient step's squared norm $\langle \delta\theta | S | \delta\theta\rangle$ exceeds a fixed threshold (hardcoded to 900), the step is rescaled down rather than discarded, guarding against ill-conditioned updates without freezing training entirely.
- an auto-rollback guard (\texttt{trainer.auto\_rollback}): if the energy becomes non-finite, or exceeds the best value seen so far by more than \texttt{trainer.rollback\_margin} (or $5\sigma$ of the local-energy variance, whichever is larger), training stops, the parameters are restored from the last good checkpoint, the learning rate is halved, and training resumes -- up to \texttt{trainer.max\_retries} times.
...
- \texttt{dim} is the dimension of the continuous Hilbert space.
- \texttt{hidden\_units} is the width of $\phi$ and $\rho$ (the encoder/decoder hidden layers).
- \texttt{out\_units} is the width of the final $\Psi$ head, split evenly between the log-modulus and phase channels.
- \texttt{lz\_proj\_K} enables the $L_z$-sector projection described in Section~\ref{sec:results}: averaging the log-amplitude over $K$ rotated copies of the input. $K=0$ or $1$ disables it.
- \texttt{pair\_hidden} enables an optional pairwise feature stream (Section~\ref{sec:results}), giving the symmetric part direct access to pairwise distances; 0 disables it.
- \texttt{pretrained\_path} loads a previous run's checkpoint to warm-start training, instead of a random initialization.
...
- an overlap diagnostic (importance sampling from an $\mathcal{N}(0,1)$ proposal) measures $|\langle \psi_{NN} | \phi \rangle|^2$ between a trained checkpoint and a chosen analytic candidate state $\phi$, used to identify which state a run has actually converged to, beyond just its energy.
- for the interacting dot, the reference energy \texttt{system.e\_ref} is obtained from an exact diagonalization in the oscillator basis (\texttt{tools/ed\_dot.py}), since no closed-form ground state exists once the Gaussian repulsion is switched on.
